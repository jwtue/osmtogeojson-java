    ...
    private fun parse() {
        for (element in data.toListOfJSONObjects().filterNotNull()) {
            when (element.getString("type")) {
                ...
                "way" -> {
                    val way = JSONObject(element.toString())
                    ways.add(way)
                    if (way.has("nodes")) {
                        way.put("nodes", JSONArray(way.getJSONArray("nodes").toString()))
                    }/* else {
                ...
                "relation" -> {
                    val rel = JSONObject(element.toString())
                    if (rel.has("members")) {
                        rel.put("members", JSONArray(rel.getJSONArray("members").toString()))
                    }
                    rels.add(rel)
                    val hasFullGeometry =
