...
import de.jonaswolf.osmtogeojson.toListOfJSONArrays
...
class OsmJsonTest : AbstractTest() {
    ...
    fun routeRelation() {
        ...
        val result = JSONObject(defaultConverter.convertOverpassJsonToGeoJson(json.toString()))
        ...
        result.put("features", result.getJSONArray("features").apply {
            put(0, getJSONObject(0).apply {
                put("geometry", getJSONObject("geometry").apply {
                    put(
                        "coordinates",
                        JSONArray(getJSONArray("coordinates").toListOfJSONArrays().filterNotNull().sortedWith(sorter))
                    )
                })
            })
        })
    ...
    fun metaData() {
        ...
        // ways and rels
        val json = JSONObject(loadFile("osm_json/meta_data_ways_rels.json"))
        val result = JSONObject(defaultConverter.convertOverpassJsonToGeoJson(json.toString()))
    ...
    fun multipolygonOuterWayTagging() {
        val json = JSONObject(loadFile("osm_json/multipolygon_outer_way_tagging.json"))
        val result = JSONObject(defaultConverter.convertOverpassJsonToGeoJson(json.toString()))
    ...
    fun multipolygonNonMatchingInnerOuterRings() {
        // complex polygon
        var json = JSONObject(loadFile("osm_json/multipolygon_non_matching_inner_outer_rings_complex.json"))
        var result = JSONObject(defaultConverter.convertOverpassJsonToGeoJson(json.toString()))
        ...
        // simple polygon
        json = JSONObject(loadFile("osm_json/multipolygon_non_matching_inner_outer_rings_simple.json"))
        result = JSONObject(defaultConverter.convertOverpassJsonToGeoJson(json.toString()))
        assertEquals(1, result.getJSONArray("features").length())
        assertEquals(
            "2",
            result.getJSONArray("features").getJSONObject(0).getJSONObject("properties").get("id").toString()
    ...
    fun multipolygonNonTrivialRingBuilding() {
        // way order
        var json = JSONObject(loadFile("osm_json/multipolygon_non_trivial_ring_building_way_order.json"))
        var result = JSONObject(defaultConverter.convertOverpassJsonToGeoJson(json.toString()))
        assertEquals(1, result.getJSONArray("features").length())
        assertEquals(
            "1",
        ...
        // way direction
        json = JSONObject(loadFile("osm_json/multipolygon_non_trivial_ring_building_way_directions.json"))
        result = JSONObject(defaultConverter.convertOverpassJsonToGeoJson(json.toString()))
        assertEquals(1, result.getJSONArray("features").length())
        assertEquals(
            "1",
    ...
    fun multipolygonUnclosedRing() {
        ...
        var json = JSONObject(loadFile("osm_json/multipolygon_unclosed_ring_nonmatching_ways.json"))
        var result = JSONObject(defaultConverter.convertOverpassJsonToGeoJson(json.toString()))
        assertEquals(1, result.getJSONArray("features").length())
        assertEquals(
            "1",
        ...
        json = JSONObject(loadFile("osm_json/multipolygon_unclosed_ring_matching_ways.json"))
        result = JSONObject(defaultConverter.convertOverpassJsonToGeoJson(json.toString()))
        assertEquals(1, result.getJSONArray("features").length())
        assertEquals(
            "1",
