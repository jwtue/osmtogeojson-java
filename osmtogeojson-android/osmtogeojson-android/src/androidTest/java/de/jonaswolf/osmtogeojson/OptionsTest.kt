...
class OptionsTest : AbstractTest() {
    ...
    fun flattenedProperties() {
        ...
        val result = JSONObject(
            flattenedConverter.convertOverpassJsonToGeoJson(json.toString())
        )
        assertEqualsJson(result.getJSONArray("features").getJSONObject(0).getJSONObject("properties"), geoJson)
    }
    ...
    fun uninterestingTags() {
        ...
        val result = JSONObject(
            tagsConverter.convertOverpassJsonToGeoJson(json)
        )
    ...
    fun polygonDetection() {
        ...
        val result = JSONObject(
            polygonFeatureConverter.convertOverpassJsonToGeoJson(json)
        )
